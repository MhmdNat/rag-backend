...
def run_web_rag_pipeline(query_text, index_name="RAGDocs", top_k=5):
    rewritten_query = rewrite_query(query_text)
    retrieved_results, _ = retrieve_top_k(rewritten_query, index_name=index_name, top_k=50)
    ...
    return {
        "run_id": str(run_helpers.get_current_run_tree().trace_id) if run_helpers.get_current_run_tree() else "",
        "query": query_text,
